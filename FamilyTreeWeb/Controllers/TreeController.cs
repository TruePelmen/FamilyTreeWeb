...
namespace FamilyTreeWeb.Controllers
{
    public class TreeController : Controller
    {
        ...
        public async Task<IActionResult> Delete(int? id)
        {
            if (id == null)
            {
                return BadRequest(); // Повертаємо помилку 400 Bad Request, якщо ідентифікатор не вказано
            }

            var tree = await _context.Trees.FindAsync(id);
            if (tree == null)
            {
                return NotFound(); // Повертаємо помилку 404 Not Found, якщо дерево з вказаним ідентифікатором не знайдено
            }
